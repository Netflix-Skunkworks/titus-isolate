...
from titus_isolate.config.constants import CPU_ALLOCATORS, CPU_ALLOCATOR_CLASS_TO_NAME_MAP


def get_cpu_allocator_index(cpu_allocator_name):
    index = -1
    try:
        name = CPU_ALLOCATOR_CLASS_TO_NAME_MAP.get(cpu_allocator_name, None)
        if name is None:
